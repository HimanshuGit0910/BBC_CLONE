...
class BusinessFragment : Fragment(), LocalServerRecyclerAdapter.OnItemClickListener {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        swipeRefreshLayout = view.findViewById<SwipeRefreshLayout>(R.id.buisnessSwipeRefreshLayout)
        progressBar = view.findViewById<ProgressBar>(R.id.buisnessProgressBar)
    ...
    private fun setRecyclerView() {
        ...
        viewModel.getLocalServerNews().observe(viewLifecycleOwner, Observer {
            ...
            inShotsRecyclerViewBusiness.adapter = adapter
            inShotsRecyclerViewBusiness.layoutManager = LinearLayoutManager(context)
            progressBar.visibility = View.GONE
            youTubePlayerView.visibility = View.VISIBLE

            val layoutAnimationController: LayoutAnimationController =
                AnimationUtils.loadLayoutAnimation(context, R.anim.layout_animation)
            inShotsRecyclerViewBusiness.layoutAnimation = layoutAnimationController

            adapter.notifyDataSetChanged()
            inShotsRecyclerViewBusiness.scheduleLayoutAnimation()
